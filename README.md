...
## 11. 阶段二：仅对 unmatched 进行 LLM 二次回填（可选）

### 11.1 处理逻辑
1. 先按本地标准表做第一阶段标准化（国家/省份/病原体/宿主）。
2. 仅对第一阶段未匹配（unmatched）的值，调用 LLM 做第二阶段回填。
3. 回填后再次走标准化器校验并写回结果。

这样可以避免“全量二次跑模型”带来的漂移。

### 11.2 已支持回填的字段
- 国家/省份：尝试把城市映射到一级行政区（如 `China|Wuhan` → `China|Hubei`）
- 病原体：尝试输出可回到字典的规范名称/别名，再映射标准 `pathogen`
- 宿主：可回填 `host_rank_1`、`host_rank_2`（例如 `children` 通常可回填为 `Human`）

### 11.3 新增参数
| 参数 | 说明 |
|---|---|
| `--enable-llm-backfill` | 开启阶段二（仅 unmatched） |
| `--llm-backfill-timeout` | 阶段二每次请求超时秒数 |
| `--llm-backfill-retries` | 阶段二每次请求重试次数 |
| `--llm-backfill-log-file` | 阶段二回填日志 CSV 输出路径 |
...
- `raw_input`（原始未匹配值）
- `resolved_value`（模型返回的回填值）
- `status`（ok/applied/error）
- `usage_count`、`applied_count`
- `error`、`llm_output`

### 11.5 本地 Windows 运行示例
```bash
python -m app.batch_extract_csv_qwen_parallel ^
  --limit 0 ^
  --workers 2 ^
...
> 使用 DeepSeek-V3 前请先关闭 Clash。

### 11.6 集群 PBS
`run_bio_task.pbs` 已更新为：
- 使用 `python -m app.batch_extract_csv_qwen_parallel`
- 已附带 `--enable-llm-backfill` 和日志输出参数
